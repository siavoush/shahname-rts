...
- **Calibration batches:** balance-engineer's tuning loop on AI-vs-AI aggregates (outcome distribution → duration p50 → first-piyade p95 → RPS); `placeholder → calibrated` promotion in balance.tres (4 anchored / 17 calibrated / 3 placeholder); affordability-table re-run on the new mine economics is first. **Precondition discovered 2026-06-10:** matches are fully deterministic across seeds (zero production randomness), so N-match batches currently have zero variance — distribution-shaped tuning needs GameRNG first; until then batches verify determinism only.
- **Sim-cost optimization triad:** spatial culling for AI scans, batched pathfinding, per-team FSM batching. **Now data-ranked** (Wave C, 2026-06-11): per-phase profiler measured the `ai` phase at 86.8% of tick cost — spatial culling for AI scans is empirically first. Roster-as-knob shipped (`--roster skirmish`, ~2.7× faster iteration); `--profile-ticks` shipped.
- **Test/CI debt — shipped 2026-06-11 (Wave C):** L7 lint (live in CI, L1–L7 at 0 violations; allowlist entries carry WHY comments), subprocess-smoke test for `--headless-batch` boot, MatchHarness v2 (13-autoload reset, real spawn catalogs, balance.tres cache pin), real-match determinism snapshot-hash test (caught + fixed Building id-counter drift). Remaining in this lane: recurring L7 allowlist prune sweep; Dictionary-probe half of §9.M7 (deferred from L7 v1); `combat_component.take_damage_x100` guard conversion (§9.B5-deferred, probe shows false branch dead at HEAD).
- **Foundation debt:** GameRNG (pulled forward by Kaveh, §4 — and now ALSO the unblocking dependency for calibration variance, see above), Telemetry/MatchLogger decision (formalize the NDJSON+log route or build the planned sink — engine-architect recommendation due before Phase-5 briefs).
