...
- **Externalize gameplay numbers — two distinct homes.** **Structural constants** (StringName tokens, paths, team enums, simulation tick rate, fixed-point scale factors, lint-allowlist patterns) live in `game/scripts/constants.gd` — agents read these as `Constants.X`. **Tunable balance numbers** (unit HP, damage, attack range, build times, costs, drain magnitudes, modifier values, dwell ticks) live in `game/data/balance.tres` (BalanceData resource per `game/scripts/data/balance_data.gd`) — agents read these via `BalanceData.economy.X` / `BalanceData.units.X` / `BalanceData.farr.X` / `BalanceData.bldg_<name>.X`. **No magic numbers in gameplay code either way.** Quick rule for "which file does this belong in?": ask *"would a designer want to tune this in a single playtest cycle?"* — if yes, BalanceData; if no, constants.gd.
