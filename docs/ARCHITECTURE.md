...
| **Translation infrastructure** | [02_IMPLEMENTATION_PLAN.md Phase 0](../02_IMPLEMENTATION_PLAN.md) | ✅ Built | — | ui-developer | Phase 0 session 4 wave 1. `translations/strings.csv` with `en` and `fa` (Farsi) columns; compiled to `strings.en.translation` and `strings.fa.translation`. Registered in `project.godot`. All HUD strings use `tr()`. Tests in `tests/unit/test_resource_hud.gd`. |
...
| **Farr HUD readout** | [01_CORE_MECHANICS.md §11](../01_CORE_MECHANICS.md) | ✅ Built | — | ui-developer | Phase 0 session 4 wave 1. `scenes/ui/resource_hud.tscn` + `scripts/ui/resource_hud.gd`. Plain-text readout: Coin / Grain / FARR / Pop. Labels bound via `tr()` for i18n. Subscribes to `EventBus` signals to refresh. Wired into `main.tscn`. Circular Farr gauge deferred to Phase 1. Tests in `tests/unit/test_resource_hud.gd`. |
